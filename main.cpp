...
typedef void(*Dllfun)(unsigned char *,unsigned char *,int ,int,int , int ,int, int , float,float,float);
//extern "C"_declspec(dllexport) void HazeRemovalUseDarkChannelPrior(unsigned char * Src,unsigned char * Dest,int Width,int Height,int Stride, int Radius ,int GuideRadius, int MaxAtom, float Omega,float Epsilon,float T0 );
//显示帧数
void draw_framecount(Mat Frame,int framecount) //Frameframecount
{
	char strFrame[10];
	sprintf(strFrame, "#%04d ",framecount);
	putText(Frame,strFrame,cvPoint(5,25),1.5,1.5,CV_RGB(255,0,0));
}
...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		{
			cout << "error: could not parse sequence gt.txt file" << endl;
			return EXIT_FAILURE;
		}
	...
	for (int frameInd = startFrame; frameInd <= endFrame; ++frameInd)
	{
		...
		{			
			...
			//Mat frameOrig;
			//Mat类型转IplImage
			Mat framemat = cv::imread(imgPath);
			IplImage *img = &IplImage(framemat);
			//用来存储去雾后的图像
			IplImage* res=cvCreateImage(cvSize(img->width,img->height),img->depth,img->nChannels);
			//去雾过程
			dehazing dehazingImg(img->width, img->height, 16, false, false, 5.0f, 1.0f, 40);
			dehazingImg.ImageHazeRemoval(img, res);
		
			Mat frameOrig(res,true);
		...
		{
			//显示当前帧数
			draw_framecount(frame,frameInd);
			imshow("frame", frame);
